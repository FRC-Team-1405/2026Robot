...
public class PidToPoseCommand extends Command {
        ...
        private PidToPoseCommand(Builder builder) {
                ...
                xController = new ProfiledPIDController(1.0, 0, 0, drivingContraints);
                yController = new ProfiledPIDController(1.0, 0, 0, drivingContraints);
                thetaController = new ProfiledPIDController(0.5, 0, 0, DEFAULT_CONSTRAINTS);
        ...
        public static class Builder {
                ...
                public Builder(Supplier<Pose2d> targetPose, Swerve drive, String commandName) {
                        this.drive = drive;
        ...
        public void initialize() {
                ...
                log("Current pose: " + drive.getPose());
                ...
                if (Math.abs(initialStateVelocity) > 0) {
                        for (int i = 0; i < 5; i++) {
                                ...
                                drive.driveFieldRelative(new ChassisSpeeds(xOutput, yOutput, 0));
                        }
                }

                commandLog.append("Initialized: " + getName());

                System.out.printf("Initializing %s to target Pose (x: %.1f, y: %.1f, rot: %.1f deg)\n",
                                getName(),
        ...
        public void execute() {
                ...
                drive.driveFieldRelative(new ChassisSpeeds(xOutput, yOutput, thetaOutput));
        ...
        public boolean isFinished() {
                double distance = drive.getPose().getTranslation().getDistance(poseToMoveTo.getTranslation());
                boolean distanceThreshold = Units.metersToInches(distance) < toleranceInches;
                Rotation2d thetaDiff = drive.getPose().getRotation().minus(poseToMoveTo.getRotation());
        ...
        public void end(boolean interrupted) {
                ...
                double finalDistance = Units
                                .metersToInches(drive.getPose().getTranslation()
                                                .getDistance(poseToMoveTo.getTranslation()));
                System.out.printf("PidToPose " + (interrupted ? "interrupted" : "completed") +
                                ". Final pose: " + drive.getPose() +
                                ", Target: " + poseToMoveTo +
                                ", Distance to target (in): " + finalDistance +
                                ", End state velocity: ( x: " + drive.getCurrentSpeeds().vxMetersPerSecond + ", y: "
                                + drive.getCurrentSpeeds().vyMetersPerSecond + " )\n");
